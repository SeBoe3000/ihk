...
function erstelleKalorien($kalorien, $protein, $kohlenhydrate, $fett) {
    $html =
        "Energie: <b>$kalorien" .
        "kcal</b>, Eiweiß: <b>$protein" .
        "g</b>, Kohlenhydrate: <b>$kohlenhydrate" .
        "g</b>, Fett: <b>$fett" .
        "g</b>";
    return $html;
}
...
function generateTOCRegex($html) {
    ...
    foreach ($matches[1] as $i => $tag) {
        // Index $i um zu passenden Text & kompletten Tag zu kommen
        ...
        if ($level > $currentLevel) {
            // neue Überschrift ist tiefer (größeres Level) --> neue UL öffnen
            $toc .= str_repeat("<ul>\n", $level - $currentLevel);
        } elseif ($level < $currentLevel) {
            // höher --> UL schließen
            $toc .= str_repeat("</li></ul>\n", $currentLevel - $level) . "</li>\n";
        } else {
            // gleiche Ebene --> vorherige LI schließen (außer beim ersten Element)
            if ($i != 0) {
                $toc .= "</li>\n";
            }
        }
